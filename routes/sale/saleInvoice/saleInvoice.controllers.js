		...
		try {
			...
			const allSaleInvoice = saleInvoices.map((item) => {
				...
				const returnAmount = returnSaleInvoice
					.filter(
						(returnSaleInvoice) =>
							returnSaleInvoice.saleInvoice_id === item.id
					)
...
const getSingleSaleInvoice = async (req, res) => {
	try {
		...
		const totalUnitMeasurement =
			singleSaleInvoice.saleInvoiceProduct.reduce(
				(acc, item) =>
					acc +
					Number(item.product.unit_measurement) *
...
const updateSingleSaleInvoice = async (req, res) => {
	try {
		const saleInvoiceProduct = req.body.saleInvoiceProduct;
		// calculate total sale price
		let totalSalePrice = 0;
		...
		prisma.saleInvoiceProduct.delete({
			where: {
		...
		const saleInvoiceProducts = saleInvoiceProduct.map(
			async (invoiceProduct) =>
				await prisma.saleInvoiceProduct.update({
					where: {
						id: Number(invoiceProduct.id),
					},
					data: {
						boxes: Number(invoiceProduct.boxes),
						length: Number(invoiceProduct.measure),
						pack_rate: Number(invoiceProduct.pack_rate),
						product_quantity: Number(
							invoiceProduct.product_quantity
						),
						product_sale_price: parseFloat(
							invoiceProduct.product_sale_price
						),
					},
				})
		);

		updatedInvoice["saleInvoiceProduct"] = saleInvoiceProducts;
